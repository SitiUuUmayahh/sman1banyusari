                ...
                @foreach($latestNews as $article)
                    <article class="group overflow-hidden rounded-xl border border-violet-100 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
                        <div class="relative aspect-[16/10] overflow-hidden bg-violet-100">
                            @if($article->gambar_cover)<img src="{{ $article->gambar_cover ? (str_starts_with($article->gambar_cover, 'http') ? $article->gambar_cover : asset('storage/' . $article->gambar_cover)) : asset('images/placeholder.png') }}" alt="{{ $article->judul }}" loading="lazy" class="h-full w-full object-cover transition duration-500 group-hover:scale-105">@else<img src="{{ $heroImage }}" alt="" loading="lazy" class="h-full w-full object-cover">@endif
